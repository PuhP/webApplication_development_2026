...
def join_queue(session_id: int):
    ...
    entry = QueueEntry(
        ...
        lab_number=max(lab_number, 1),
        work_count=max(work_count, 1),
        estimated_minutes=max(min(estimated_minutes, 180), 1),
...
def teacher_sessions():
    ...
    if request.method == "POST":
        ...
        if action == "subject":
            ...
            subject = Subject(name=request.form.get("name", "").strip(), group=group, teacher_id=int(teacher_id))
            if not subject.name:
            ...
            session = QueueSession(
                ...
                title=request.form.get("title", "Очередь на защиту").strip() or "Очередь на защиту",
...
def toggle_session(session_id: int):
    ...
    db.session.commit()
    broadcast_session_update(session.id)
    flash("Статус очереди изменен.", "success")
    return redirect(url_for("main.teacher_sessions"))
...
def profile():
    ...
    return render_template("profile.html", subjects=subjects, uploads=uploads, grades=grades)
